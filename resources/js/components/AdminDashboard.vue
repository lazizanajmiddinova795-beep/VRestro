<template>
  <div class="min-h-screen bg-transparent text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
    ...
    <main class="flex-grow max-w-7xl w-full mx-auto px-6 py-8 relative">
      ...
      <div class="flex items-center space-x-1 bg-slate-950/40 border border-white/5 rounded-2xl p-1 mb-8 shrink-0 w-max">
        <button 
          @click="switchTab('overview')"
          class="px-5 py-2.5 rounded-xl text-xs font-bold transition duration-205"
          :class="activeTab === 'overview' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-white'"
        >
          Umumiy Boshqaruv
        </button>
        <button 
          ...
          class="px-5 py-2.5 rounded-xl text-xs font-bold transition duration-205"
          :class="activeTab === 'analytics' ? 'bg-indigo-600 text-white shadow-md' : 'text-slate-400 hover:text-white'"
        >
      ...
      <div v-if="activeTab === 'analytics'" class="space-y-6">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h2 class="text-xl font-bold tracking-tight text-white">Hisobotlar va Tahlillar (BI Suite)</h2>
            <p class="text-xs text-slate-400">Restoranning moliyaviy oqimlari, menyu tahlili, ombor sarfi va xodimlar samaradorligi monitoringi.</p>
          </div>
          ...
          <div class="flex flex-wrap items-center gap-3">
            ...
            <button 
              ...
              class="flex items-center space-x-2 px-4 py-2 rounded-xl bg-gradient-to-r from-violet-600 to-indigo-600 text-xs font-semibold text-white shadow-lg shadow-indigo-600/30 hover:scale-[1.02] transition duration-200"
            >
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div class="relative overflow-hidden rounded-2xl border border-white/5 bg-slate-950/40 p-6 backdrop-blur-xl shadow-xl animate-fadeIn">
            ...
            <div v-else class="space-y-4.5">
              <div v-for="(food, idx) in reportsStore.menuReport.top_selling" :key="food.id" class="space-y-1.5">
                <div class="flex justify-between text-sm">
                  ...
                  <span class="text-indigo-400 font-bold">{{ food.units_sold }} dona ({{ formatCurrencyBI(food.revenue) }})</span>
                </div>
            ...
            <div v-else class="space-y-3">
              ...
              >
                ...
                <div class="text-right">
                  <span class="text-xs text-red-400 font-bold block">{{ formatCurrencyBI(food.revenue) }}</span>
                </div>
              </div>
            </div>
          </div>
